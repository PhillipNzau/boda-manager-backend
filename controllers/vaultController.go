...
func CreateVaultItem(c *gin.Context) {
    ...
    _, err := config.GetCollection("boda_manager").InsertOne(context.Background(), item)
    if err != nil {
...
func GetVaultItems(c *gin.Context) {
    ...
    cur, err := config.GetCollection("boda_manager").Find(context.Background(), bson.M{"user_id": userID})
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
        return
    }
...
func GetVaultItem(c *gin.Context) {
    ...
    var item models.VaultItem
    err := config.GetCollection("boda_manager").FindOne(context.Background(), bson.M{
        "_id": id,
        "user_id": userID,
    }).Decode(&item)
    if err != nil {
...
func UpdateVaultItem(c *gin.Context) {
    ...
    res, err := config.GetCollection("boda_manager").UpdateOne(context.Background(),
...
func DeleteVaultItem(c *gin.Context) {
    ...
    res, err := config.GetCollection("boda_manager").DeleteOne(context.Background(), bson.M{"_id": id, "user_id": userID})
